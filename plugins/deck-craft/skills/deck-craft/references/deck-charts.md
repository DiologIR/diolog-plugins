...
A two-bar group sits below the gate's three-bar minimum, so it is never judged automatically. Carry the shared scale in the `aria-label` and check the arithmetic yourself: both widths are percentages of the same maximum, so `0.4 / 8.0 = 5%` is the whole proof.

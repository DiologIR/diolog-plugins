...
§7 opens with *"Automatic Preflight & Review Execution is MANDATORY — never wait for the
user to ask"*, "you MUST", "you must never ask". The instruction it carries is correct and
load-bearing; the register is the one Google says stops helping and can hurt. Two
consequences for you:

- **Read it as a plain rule.** Run `run-preflight.sh` as a build step. Fix every blocker.
  The capitals add no information, so give them no extra weight — and in particular don't
  read the emphasis as licence to substitute urgency for the run.

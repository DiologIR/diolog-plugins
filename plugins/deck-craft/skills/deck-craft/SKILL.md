...
**Automatic Preflight & Review Execution is MANDATORY — never wait for the user to ask.** Whenever building or updating an HTML presentation deck, you MUST automatically run `scripts/run-preflight.sh <url> [--regulated]` as an integrated step of the build pipeline before declaring the deck finished. You must never ask the user whether to perform a review; it is an invariable part of deck-craft. Any blocker (`stageGeometry > 0`, `stageContentOverflow > 0`, `titleWrap > 0`, `cardOverflow > 0`, `textOverlaps > 0`, `overflow > 0`, `chromeCollisions > 0`, `inkPastSlide > 0`, `provenanceMissing > 0`) requires an immediate code fix and re-run.

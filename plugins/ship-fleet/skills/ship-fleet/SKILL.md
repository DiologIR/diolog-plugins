...
| Evidence lenses (work Phase D UI-fidelity / clause-table / reachability), adversarial finding-verifiers, e2e Phases 0–4, design-craft leaf verifiers + page assembly from existing composites, triage Sentinel verdict + Assumptions (with the assumptions gate), plan synthesis Trivial/Small | **Sonnet** | Structured, oracle-checked work under a stronger reviewer. Sonnet ≈ 80% of Opus cost per task (it spends more tokens; measured 2026-07 on Sonnet 4.6 — Sonnet 5's new tokenizer ≈ +30% tokens and intro pricing change the ratio, so re-sweep per model-and-effort §4 on any model change) — pick it for adequacy, not savings |

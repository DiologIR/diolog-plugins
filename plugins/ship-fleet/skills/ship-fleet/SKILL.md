...
**The Codex `max` gate row is not a cost lane and does not follow those economics.** It buys *independence*, not savings: three checks — the triage spec review, the plan review gate, and work Phase D's completeness critic — route out of Claude's family precisely because a same-family reviewer shares the blind spot that let the defect through. They are **mandatory where Codex is available**, exempt from the kill-switch (a reviewer that keeps finding real defects is working, not thrashing), and their fallback is a **logged downgrade in the artifact**, never a silent pass. Availability — a missing binary, no login, a usage or rate limit — is the only licensed reason to fall back; check it once at fleet start (`references/preflight.md`) and note the outcome in ORCHESTRATOR.md so every runner inherits the same picture.

---
name: improve-prompt
description: "Review and improve any prompt artifact — task prompts, prompt templates, system prompts, agent/persona instructions, CLAUDE.md rule files, and Claude Code skills (SKILL.md) — grounded in Anthropic's bundled prompt-engineering docs and the Karpathy coding-behaviour principles. Diagnoses against a structured rubric (clarity, motivation, examples, XML structure, output contract, hallucination guards, consistency, injection/leak guardrails, token weight, stale model patterns; plus trigger-description and progressive-disclosure checks for skills), then rewrites surgically with a per-change rationale citing the principle behind it. Use whenever the user wants a prompt improved, reviewed, hardened, optimized, fixed, tightened, or modernized — 'improve this prompt', 'why does Claude ignore this instruction', 'my agent hallucinates/leaks/drifts', 'review my system prompt', 'harden this against prompt injection', 'update this prompt for Opus 4.8 / newer models', 'make this skill trigger better', 'tighten up this SKILL.md' — even when they paste a prompt and just say 'make this better'. Do NOT use for creating a brand-new skill from scratch with test evals (use skill-creator), writing content in a person's voice (use their content skill), or improving product code rather than prompts (use code-review or /improve)."
allowed-tools:
  - "Read"
  - "Write"
  - "Edit"

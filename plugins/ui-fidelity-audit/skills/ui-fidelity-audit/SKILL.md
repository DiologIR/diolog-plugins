...
- **Discover the FULL surface list — the names you were given are seeds, not the scope.** Every trigger on a seed surface that opens a *new surface* — a modal, drawer, sheet, popover, menu, picker, command-palette, intake, or wizard — is **its own surface and must be audited on both sides**, even though nobody listed it. *A surface you never enumerated is a surface you never audited* — this is exactly how a whole "Send to agent" / "Create issue" / command-palette modal ships divergent: it was never on the list. Build the real list two ways and union them:
  - **At render:** walk every interactive control on the seed surface (the probe's `controls`) and note what each *opens* — the kebab, the primary CTA ("Send to agent"), a "+ New", a property row that fires a picker. Each target is a surface to add.
  - **In code:** grep the seed surface's files for mounted overlays — `role="dialog"`, `<Dialog`/`<Drawer`/`<Sheet`/`<Popover`/`<Modal`, `*Modal`/`*Intake`/`*Picker`/`CommandPalette`, and conditional `{open && …}` overlay mounts.

  The reference opens the same set; diff each spawned surface on both sides like any other (render it, probe it, run 3A then 3B). **Infer wider scope wherever a trigger leads somewhere** — keep expanding the list until you hit the feature's genuine boundary, then state which spawned surfaces you deliberately excluded and why. The named list is the floor, never the ceiling.
...
- **Container spacing** — diff each region container's **padding / margin / gap** (the probe's `style.pad`, `margin`, `gap`), not merely whether the wrapper exists. A region with the right background and border but extra vertical `padding` (or a label `margin`, or a wider `gap`) reads as "has its wrapper ✓" to a presence check yet sits looser/tighter than the reference — this is the spacing that *silently survives* when you only ask "is the wrapper there?". Exact alignment is mockup-align's job, so **flag** the drift here with the measured numbers; do not let it fall between the structural pass (which excludes padding) and a hand-off that never happens.
...
- **Interaction architecture** — what each trigger opens (modal vs drawer vs popover) and where it lives in the DOM. A pixel-perfect element in the wrong container is still wrong. **And what it opens is itself a surface — enumerate it in Step 1's surface-discovery and audit it on both sides; don't just note that a trigger opens something and move on.** (mockup-align's Phase 0 covers this in depth once you hand off.)
...
11. **Scope-by-the-named-list / spawned-surface blindness** — you audited the surfaces in the brief and never the modals/drawers/popovers/pickers/command-palettes those surfaces *open*. An unlisted surface is an unaudited one: a whole "Send to agent" or "Create issue" modal ships divergent because it was never on the list, and "I audited the 7 surfaces you named" feels thorough while missing everything they spawn. The named surfaces are **seeds** — enumerate every trigger's target (Step 1's surface-discovery) and audit it too; infer wider scope wherever a trigger leads somewhere.
12. **Spacing deferred, then never aligned** — the structural pass correctly excludes padding (placement first), the styling pass flags spacing "for mockup-align", mockup-align is skipped (cost, time, "later"), and the spacing drift ships looking *done*. The outer wrapper can match (same 16px aside) while an inner region carries the wrong vertical padding / label margin / gap. Spacing isn't done until it is **measured and aligned**; if the aligner never ran, mark every spacing row **flagged — not resolved** and never call the surface a spacing match on structure alone.
...
- **the surface list was inferred, not just taken (Step 1): every modal/drawer/popover/picker/command-palette a seed surface opens was enumerated and audited on both sides, or explicitly excluded with a stated reason — "I audited the surfaces named" is not enough;**
- every surface in scope (including each gated modal/drawer/expanded state and every spawned surface) was **rendered** on both sides;
...
- confirmed stylistic gaps are queued for **mockup-align** and structural gaps have an owner/plan; **every spacing/padding/margin row is either aligned (mockup-align actually ran) or marked `flagged — not resolved` — no surface is reported as a spacing match on structure alone.**

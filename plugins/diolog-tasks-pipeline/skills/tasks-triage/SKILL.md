...
4. **Ground in the codebase (mandatory).** Use `Glob`/`Grep`/`Read` to locate every component, page, service, route, or feature the ticket references. Detect ambiguous matches (one name → multiple locations) and naming mismatches (UI label vs route/component name). Map the affected files. Do your technical reasoning internally — it informs the comment but never appears in it. **Source reading tells you what the code says, never what the app shows.** Any statement you put in front of the human about the product's *current rendered state* ("these pop-ups already use the standard heading style", "the label already wraps") must be either observed in the running app or phrased as unverified ("the code suggests… — the planner should confirm on screen"). A class string is not a rendered fact — overrides get silently discarded — and an Essential Question built on a false grounded claim gets a confidently wrong answer that the whole pipeline then builds on.
...
6. **Codex cross-family review of the drafted triage (mandatory where available) — before you post.** Draft the comment, then hand it plus the ticket to a reviewer **outside Claude's model family**: the Codex CLI running `gpt-5.6-sol` at **`max`** reasoning effort, read-only, grounded in the actual codebase. Everything else in this pipeline is Claude reviewing Claude, and this catches what that family is blind to — a readiness verdict whose logic doesn't close, or a "grounded" assumption that names code which doesn't behave the way the draft claims.
   ...
   **Then act — running the review is not the gate; acting is.** Per finding: **accept** it and revise the draft; **reject** it with a stated reason (it contradicts a human's authoritative reply, it expands scope the ticket never asked for, or you checked the code and it's wrong); or **escalate** — a `Critical`/`High` finding exposing a genuine **external** dependency becomes an Essential Question and the issue goes to `Needs More Info`. Never post a `MATERIAL DEFECTS` draft unrevised. A finding adopted without checking is how a ticket acquires requirements nobody asked for. **Egress + the repo opt-out.** `-s read-only` restricts writes, not the network: the ticket text and every source file the reviewer opens are transmitted to OpenAI. So before the call, grep `CLAUDE.md` / `AGENTS.md` / `ORCHESTRATOR.md` for `ANTHROPIC-ONLY`, `NO EXTERNAL MODEL CLIS`, or `external-model-clis: off` — a hit means this repo **opted out**: run the in-family reviewer and log `codex: opted out (<file>) → claude`, which is a correct run, not a degraded one. Re-check per invocation; it is the only kill-switch that reaches a run already in flight. If the lane is unavailable instead (no binary, not logged in, usage/rate limit, empty output, deadline fired, repeated errors), fall back the same way and **say so in your final summary** — availability and the opt-out are the only licensed skips. Bound the call (`perl -e 'alarm shift @ARGV; exec @ARGV' 600 codex exec …`) and verify `reasoning effort: max` in the captured log; a dropped flag silently inherits the user's config default.

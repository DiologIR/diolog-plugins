...
**This gate is a CONFORMANCE check, not a second correctness review.** Its question is whether the phase drifted from, dropped, or half-built a stated requirement — answered with evidence (the clause, the `file:line`, the gate you actually ran), which is what re-reading your own diff cannot give you. Don't stack a general "review it again" round on a phase that already reviewed itself; see the generalized twin's `feature-spec-pipeline/skills/work/references/model-and-effort.md` §6.
...
**Keep the comment to its shape** — tables, counts, assumptions, drops, gate results; not a narrative of the phases (twin's `model-and-effort.md` §7).

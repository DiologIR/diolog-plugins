...
5. **Scope-narrowing check (ALL tiers, mechanical — a grep and a diff, not a model).** Before any status move, compare every "Out of scope" line and every requirement the plan does **not** carry against (a) the ticket's own sentences and (b) the triage Assumptions. Any overlap is a narrowing, and silence in the plan file is not disclosure — the ticket thread is. Post it as a one-line comment: *"The plan excludes <X>; assumption <N> ('<text>') appears to include it — reply to keep it excluded, or I'll plan it in."* This check exists because the review gate below is skipped for Trivial/Small tiers, and small tickets are where a quiet descope otherwise meets no reader before the worker inherits it as a premise.

6. **Plan review gate — Codex cross-family review, before the status moves (Standard and Large tiers; skip for Trivial/Small).** The plan is the artifact every later stage amplifies, so its reviewer comes from **outside Claude's model family**. Run it after the file is written and before step 7:
   ...
   - **Cross-family one-shot review — the Codex CLI, `gpt-5.6-sol` at `max` effort (mandatory where available).** Read-only and grounded in the real codebase:
     ...
     Codex has no Tasks access, so write the ticket description + the full comment thread to a scratch markdown file and name it in the prompt alongside `docs/plans/<id>.md`. Full mechanics — availability check, the verbatim prompt contract (R1), finding disposition, fallback — are in the generalized twin's `feature-spec-pipeline/skills/work/references/codex-cli.md`; follow it rather than re-deriving the invocation. `read-only` so the reviewer cannot "helpfully" fix the plan it reviews; pass `-m` and the effort **explicitly** (`~/.codex/config.toml` may default lower); `< /dev/null` or it waits on stdin. **Know the gate's limit:** a read-only reviewer verifies what code *says*, never what it *renders* — it cannot catch a false claim about rendered appearance, so never count it as behavioural evidence (see the claim-marking rule below).
...
7. **Commit the plan, then post the Tasks comment with the repo-relative path + the commit (do NOT upload the file).** First commit the plan file in the main working tree — `git add docs/plans/<id>.md && git commit -m "docs(plans): <id> implementation plan"` (do NOT push; local commit only) — so the artifact the pipeline builds from is auditable and provably in the repo, like every other plan. Then via `mcp__diolog-tasks__create_comment`, post:

   > Implementation plan written to `docs/plans/<id>.md` (committed: `<short sha>`). — Claude (AI Assistant)

   Never post "(in the repo)" for an uncommitted file — the claim must be checkable against `git ls-files`. Do **not** base64-encode, do **not** call `mcp__diolog-tasks__add_issue_link`, do **not** attach the file. The comment carries only the repository-relative path + sha so a developer (or the `tasks-worker` skill) can open it from the repo. (Rationale: the file lives in the repo with the code and is read from there; an uploaded copy would immediately drift from the in-repo source of truth.)

8. **Move status** (skip in dry-run). Call `mcp__diolog-tasks__update_issue` with the `Ready for AI` state ID and verify the response shows it. Skip only if the issue is already in `Ready for AI`, `Developer Review`, `In Progress`, or any further-downstream state — never downgrade.

9. Print a short summary (tier + the repo-relative plan path + commit sha + the review gate's verdict and accept/reject tally, or `codex: unavailable → claude`). In dry-run, say the file was written locally (uncommitted) and no Tasks writes were made — the gate is read-only, so it still runs.
...
- **Mark rendered-appearance claims MEASURED or ASSUMED.** Glob/Grep verifies that code exists and what it says — never what it renders. Any plan statement about how something currently *looks or behaves on screen* ("the assign and share modals already use it correctly", "the heading is 20px serif") must carry `(measured: <getComputedStyle/browser evidence>)` or `(assumed from source — verify in browser before building on it)`. A class string is not a rendered fact: overrides get discarded (merge configs, specificity, inline styles), and a plan premise read off a class string is how a false "reference implementation" becomes the worker's unchallenged truth.

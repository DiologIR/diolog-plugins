---
name: ship-feature
description: End-to-end feature delivery conductor for the Diolog product — takes ONE feature (inline description or a markdown file) and drives it from a bare idea all the way to a merged, tested, production-ready feature by orchestrating the existing skills in order, in a single in-session flow. It runs design-craft to represent the feature's WHOLE UI in the project's design-system elements/composites/mock-pages (grounded in apps/web-design-system plus an optional DESIGN.md design-language spec when provided or present, and any provided pure HTML/CSS/JS mock), then triage (→ docs/specs/spec-DIO-XXXX.md), plan (→ docs/plans/plan-DIO-XXXX.md), work (implement in an isolated git worktree), a deferred-work loop (re-work, or child triage+plan+work for extra phases — all on the SAME branch), gap-fix (close remaining spec gaps in code), acceptance-e2e (a comprehensive Playwright suite over every user flow / action / interaction / menu, run against the feature branch locally and fixed), and finally commit → rebase → merge into the integration branch → push → clean up the worktree(s). The conductor stays in-session so it keeps the accumulated feature context across every stage, and leans on the on-disk spec/plan/mock artifacts as durable, resumable memory. Use this whenever someone wants to take a feature ALL THE WAY — "ship this feature end to end", "run the full pipeline on <feature>", "take this feature from description to merged", "build and ship <feature.md>", "design, spec, build, test and merge <feature>", "do the whole feature for me" — even if they don't name the individual skills. Prefer this over invoking triage/plan/work/gap-fix/acceptance-e2e one at a time whenever the goal is a finished, merged feature rather than a single stage. Not for a single stage (use that skill directly) and not for non-Diolog repos (it assumes the Diolog design-system + worktree + docs conventions).
---

# Ship Feature — end-to-end feature conductor

Take **one** feature from a bare description to **merged, tested, production code** by conducting the existing Diolog skills in sequence, in a single in-session flow. You are the **conductor**: you hold the feature's intent from the first read to the final merge, invoke each stage-skill in turn, verify its output, carry the right context forward, and make the judgment calls *between* stages that no single stage can make.
...
- The **target repository** is the Diolog product repo — run from its root. It provides `CLAUDE.md`, the design system (`apps/web-design-system` + its `tokens/tokens.css`, plus an optional `DESIGN.md` design-language spec when one is provided or present in the repo), the `docs/` tree + `docs/feature-specs/LEDGER.md`, the e2e harness (`apps/web/e2e`), and the integration branch (`origin/staging`, else the repo default).
...
- **Docs stay in the main tree; code stays on the branch.** The spec/plan/ledger are the pipeline's Linear-replacement, read from the main working tree — never commit them onto the feature branch. The design-system mock UI is code and rides the branch (Phase 1 reference).

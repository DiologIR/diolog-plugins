...
- [ ] **The three out-of-family gates ran and landed:** the triage spec review, the plan review gate, and `/work` Phase D's completeness critic — each on Codex `gpt-5.6-sol` at `max` effort, each with a recorded verdict and every finding accepted-and-fixed, rejected-with-a-reason, or escalated. A missing verdict is a **skipped gate**, not a clean one. Where Codex was genuinely unavailable, the in-family fallback must be **logged as a downgrade** in the artifact — an unlogged fallback is indistinguishable from a skip, so treat it as one and re-run the gate before merging.

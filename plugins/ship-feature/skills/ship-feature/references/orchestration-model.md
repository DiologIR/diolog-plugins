...
| Mechanical work Phase B/E slices meeting the delegation criteria | **codex `gpt-5.6-sol` at `medium`** (the default executor — `feature-spec-pipeline/skills/work/references/codex-cli.md`, role R3); the older cheap-executor CLI lanes (composer-2.5 / glm-5.2-high) remain available on the same terms — ship-fleet's `references/cursor-composer.md` |

...
Do this with the **bundled differ, not by eye** (`assets/diff/` — see its README): extract the mock's computed styles (`extract-mock.js` via `playwright-cli eval`), point it at the target dump (RN harness `_latest.json`, or an `extract-mock` dump of a React/DOM route), and run `node diff.mjs --mock … --app … [--anchor "<screen title>"] → report.md`. The report lists ❌ mismatches (element · property · target vs mock), ⚠︎ probes present in the mock but unmatched in the target (a real absence **or** an intentional content/icon-ligature swap — you classify each), and ✓ matches. For React/HTML both sides are `getComputedStyle`; for React Native the reference is `getComputedStyle` and the target is the resolved style props from the rendered tree (`references/react-native.md`). Geometry is compared only on the gutter an element is anchored to (left-inset for left-anchored, right-inset for right-anchored) because the mock frame width ≠ the device width. RN allows one shadow per view, so a two-layer CSS box-shadow can't be fully reproduced — note the approximation rather than chasing it. **Work the report to zero unexplained mismatches** — that, not a glance at the artifacts, is the style verdict.
...
- structure was diffed before styling; the per-property computed-style comparison was run by the **bundled differ** (not by eye), and each in-scope screen's `report.md` reaches **zero unexplained mismatches** — every ⚠︎ unmatched line classified as a real absence or an intentional content/icon swap;

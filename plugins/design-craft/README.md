...
`visitor-modes` · `motion-design` · `gsap-motion` · `depth-and-3d` · `laws-of-composition` · `typesetting` · `mobile-design` · `data-viz`
...
**Visitor modes, contracts, and bounded verification (v1.10).** Every surface now names what its visitor is there to do — **Persuade / Operate / Read / Experience**, chosen from the *surface* rather than the product — and `visitor-modes` carries the Operate/Read depth that deliberately inverts this skill's Persuade-tuned defaults (earned familiarity over invention, one type family, fixed rem scale, a state-rich semantic vocabulary, no page-load choreography). Direction-setting gains an anti-convergence procedure: name the category's rut *and* its predictable opposite before generating, derive seven candidates from the audience's cultural world spanning at least three material families, and don't build your own top-ranked one — it's what every run would ship. Every direction round carries a standing exit (the category standard, played straight, executed at full fidelity if taken). The chosen direction is written into the artifact as a five-block contract (THESIS / OWN-WORLD / STORY / FIRST VIEWPORT / FORM, plus a FINISH line) that the critique gate audits promise by promise, with mandatory TYPE and MATERIAL rows — a face of the wrong character or flat CSS standing where the direction promised real material is *contradicted*, however well the layout matches. Reviews now compute a disposition (`ship` / `fix` / `rebuild`) reported verbatim, escalate to a rebuild directive rather than laundering a wholesale failure into a patch list, and score each repair `resolved` / `partial` / `unresolved` against the recapture, because narration is not evidence. Verification is bounded: batched rounds, desktop and mobile together, stop when a round resolves nothing.
...
- **Anti-AI-slop by construction** — `ai-slop-check` detects and replaces the template tropes (including all three current AI-default looks: warm-editorial, dark + acid accent, broadsheet, plus imitation material, system display faces, glyph-as-icon and the reflex modal), and `frontend-aesthetic-direction` pre-empts the default-model look with rut-naming, subject-derived candidates spanning multiple material families, a committed signature element, and a swap-test genericness gate before any hi-fi work.
- **Modes, not one register** — `visitor-modes` stops Persuade-tuned craft rules being applied to a task surface (and vice versa); an Operate surface's failure mode is strangeness without purpose, not flatness, and the skill reviews for both.

...
- **Overlays escape their container** — an absolutely-positioned dropdown inside an `overflow: hidden`/`auto` ancestor gets clipped; use `<dialog>`, the popover API, `position: fixed`, or a portal.

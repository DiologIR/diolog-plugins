...
2. **Check for color-only signaling.** Flag any state communicated by color alone — green/red without an icon, blue link with no underline, chart with no legend or text labels.3. **Check for difficult color combinations.** Red+green (most common colorblindness), blue+yellow at similar lightness, light gray on white, colored text on colored backgrounds with similar brightness.

...
1. **Verify text contrast.** Normal text (under 18px) needs 4.5:1; large text (18px+ bold or 24px+) needs 3:1; UI components (buttons, icons, focus rings) need 3:1. Thresholds are **inclusive with no rounding**: exactly 4.5:1 passes, 4.499:1 fails — rounding up is not a permitted mechanism. Compute the actual ratio for any color pair you can resolve (resolved hex values, tokens followed back to their source). Flag every failing pair with the ratio and the required minimum.

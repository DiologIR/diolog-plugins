...
Walk the deck top to bottom in a browser. Check: scaling works at multiple viewport sizes; the counter increments correctly; keyboard nav (arrows, space) works; no content overflows the slide bounds; type sizes meet the minimums (24px+ body); color contrast meets WCAG (quick check, or run `accessibility-audit.md` for thoroughness). For end-of-turn delivery, surface the final HTML file and do the thorough pass yourself — render, screenshot each slide, open every capture, probe nav. A deck is a handful of tool calls to walk; it isn't work to hand off.

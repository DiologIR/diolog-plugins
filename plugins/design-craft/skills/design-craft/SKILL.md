...
- **Browser verification** (render, DOM, console, screenshots) uses whatever automation is available — Playwright, `playwright-cli`, or the Chrome MCP — and you drive it yourself. `references/visual-verification.md` is the playbook: viewport matrix, capture-once-crop-many, DPR by purpose, before/after pairing, overflow probe. Inspect crops at DPR 2–3, not page thumbnails: a defect you can't resolve is a defect you'll clear.

...
**Five CSS mechanics that silently break components — each cost a real render bug invisible in source.** The common defence is the same in all five: **verify the computed value on the node, never the presence of the rule.** A CSS fix that lost the cascade and a CSS fix that was never written are indistinguishable in source.

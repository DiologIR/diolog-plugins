---
title: macOS HIG Reference Library — Index
role: index
---

# macOS HIG Reference Library

A practical, build-oriented distillation of Apple's **Human Interface Guidelines
for macOS** (macOS 26 "Tahoe" / 27 era), mirroring the macOS-relevant sections of
the [HIG left sidebar](https://developer.apple.com/design/human-interface-guidelines/).
Each file is a focused reference for implementing one part of a **native-feeling
macOS app** — what to use, how it behaves, the concrete metrics, and (critically)
the **common non-native mistakes** web/Electron apps make.
...
`lists-and-tables` + `toolbars` + `the-menu-bar`). The design **tokens** (the
concrete colour/type/material values) live in the theme spec:
**[../../../../DESIGN.md](../../../../DESIGN.md)** (the macOS 27 *dark* theme) and
its light sibling **[../DESIGN.md](../DESIGN.md)**, both sourced from the vendored
**[../apple-macos-27-ui-kit.tokens.json](../apple-macos-27-ui-kit.tokens.json)**.

**Tagging convention.** Inside each file: **`[HIG]`** = Apple's stated guidance ·
**`[macOS 27 UI Kit]`** = a concrete value from the vendored token export ·
**convention / `[secondary]`** = a community-derived default to measure, not
assume (Apple doesn't publish most pixel/blur/timing numbers). The kit is a
next-gen, third-party export — a strong reference, **verify before shipping**.

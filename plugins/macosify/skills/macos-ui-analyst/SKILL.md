...
description: "Analyse a static macOS 26 (Tahoe / Liquid Glass) UI screenshot into a structured, native-ready specification — a macOS Native Translation Report with a semantic AX-tree component inventory, SwiftUI + AppKit mapping, design tokens, and an epistemic-tagged build guide a downstream generator can compile to authentic native code. It works semantics-before-pixels (builds the accessibility tree before reasoning about colour), enforces the Liquid Glass two-layer rule, separates observed measurements from inferred conventions, and classifies the screenshot's framework lineage — true AppKit-native vs Mac Catalyst vs 'Designed for iPad' iOS-on-Apple-Silicon vs SwiftUI — always treating AppKit-native as the gold standard and flagging iOS-derived tells with their native correction. After each analysis it MERGES durable, generalisable learnings into one accumulating knowledge file (learnings/macos-ui-learnings.md) that the macosify skill consumes. Use this skill whenever the user shares or points at a macOS screenshot and asks to analyse it, identify what's native or not-native about a Mac UI, extract its structure, reverse-engineer it into SwiftUI/AppKit, or build up macOS UI knowledge from real screenshots."

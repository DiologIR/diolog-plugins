---
name: macosify
description: "Refit an existing UI so it feels authentically native to macOS 26 (Tahoe / Liquid Glass) — real window chrome and traffic lights, SF Pro at the 13pt macOS body scale, dynamic semantic colour bound to the user's accent, Liquid Glass only on floating chrome, concentric corners, native lists/tables instead of card grids and per-item buttons, the menu bar as the complete command surface, and the macOS keyboard/pointer model. Crucially it leans to TRUE AppKit-native macOS and corrects iOS-derived looks — Mac Catalyst and 'Designed for iPad' apps on Apple Silicon that produce an iOS-like UI on macOS — toward native AppKit idioms. Use this skill whenever the user asks to 'macosify' something, 'make this feel like a native Mac app', 'this looks like a web app / an iOS app — nativise it for macOS', 'refit/redesign X to macOS conventions', 'fix what's not native about this Mac UI', or to make a built surface match the Mac platform. Grounds every decision in the bundled macOS 26 HIG library, a token-level DESIGN.md, and an accumulating learnings file from real-screenshot analysis. For analysing a static screenshot into a structured report (rather than refitting live code), use the macos-ui-analyst skill instead."

...
Loads for diffs under a mobile app package (`apps/mobile/**`, `*.native.tsx`, React Navigation / Expo Router files, `app.json`/`app.config.*`, `metro.config.*`). React component-quality and hook rules from `frontend-web-checklist.md` §1–3 apply to RN too — load both; this file covers only what is different on native.
...
- **Sensitive data in AsyncStorage**: tokens, PII, or auth state in plain AsyncStorage where the repo has SecureStore/Keychain — HIGH.

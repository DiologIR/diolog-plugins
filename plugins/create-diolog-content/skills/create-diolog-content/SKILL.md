...
Load **`references/diolog-voice.md` (always, the base layer) plus the one matching persona file**, and consult **`references/diolog-voice-review.md`** (the reviewer-correction ledger) when a phrasing or naming judgement is close - the owner enabled it for drafting and review from v1.5.0, after the package's blind-testing phase closed. Don't load personas you aren't using. The rules remain the standard: recognising a document or its review history is never itself a finding, and a revision still runs the item-by-item disposition pass against the feedback actually supplied rather than replaying remembered history.
...
- **Rules first, history second.** The ledger informs judgement, but artifacts are judged by the rules. Recognising a previously reviewed document is not a finding and is never announced as one, and a revision dispositions every item of the supplied feedback rather than trusting recall of it.

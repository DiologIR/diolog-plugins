...
**In the drafting and review load path by owner decision (2026-07), now that the package's blind-testing phase has closed.** Consult it when a phrasing or naming judgement is close: reviewer rulings here settle choices the general rules leave open. Two guards survive from the testing phase: the rules are still the standard (recognising a previously reviewed document is never itself a finding, and is never announced as one), and a revision still dispositions every item of the feedback actually supplied rather than replaying this history from memory.

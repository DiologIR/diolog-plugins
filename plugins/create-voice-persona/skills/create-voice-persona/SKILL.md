...
Hand over: where the package lives, the file map, how to invoke it (skill trigger phrases for the plugin shape; "attach these files as context" for the docs shape), the lint command, and a short honest note listing what's marked `[Uncertain]` and which registers had thin evidence. Remind the owner they remain the author: the persona drafts, they review and publish. Offer a first test drive — drafting one real piece through the new package — as the natural next step, and tell the owner their line-edits on those early drafts are the package's best calibration data: send them back and the package absorbs them (Step 6).

## Step 6 — Maintain the package from owner feedback

When the voice owner reviews the persona's real output and reacts ("I would never say X", "this still feels AI", "stop using that phrase"), update the *package*, not just the draft, following the "Maintaining a package from owner feedback" section of `references/package-blueprint.md`: owner corrections are the highest-grade evidence (mark them `[Source: owner review YYYY-MM]`; they outrank `[Inference]` and even corpus-derived rules the owner overrides); generalise each correction to the failure class it exemplifies and state the rule once in the file that owns that class; encode never-say phrases in `voice-lint.json`; move overexposed signature phrases to a retired list rather than deleting the evidence; and rewrite any worked example that demonstrates the old behaviour — a stale example silently outvotes the new rule. Bump the package version.
...
- **Length is a voice trait, and it is the one the model overrides silently.** Current models default to longer output than most people write, and that default is not reachable by turning reasoning effort down — only by an explicit instruction. So every variant carries a stated length target in its own units (a word range, a paragraph count, "two sentences") plus the shape that target implies; a LinkedIn post at twice its owner's real length fails the voice regardless of how well the diction matches. Extract the target from the corpus (median and range per register), don't guess it.
- **Re-baseline the package when the underlying model changes.** A voice package is calibrated against whatever prose baseline it was written on, and that baseline moves between model generations — default length, register, and hedging habits all shift. On a model change, re-run the lint over every worked example and re-read one sample anchor beside a fresh draft of the same brief: if the drafts have drifted, the fix is a tightened rule in the package, not a note in the chat. Record the baseline the package was calibrated on so the next person knows when it went stale.
...
- **Signature phrases are recognition markers, not generation quotas.** Every generated package rations them (at most once per piece, never in consecutive pieces, never the default close) and carries a retired-phrases mechanism — owners hear repetition as self-parody long before an outside reader does.
- **The fingerprint is a distribution, not a formula.** Generated packages must forbid mechanical rhythm imitation (the short-sentence-then-long-sentence metronome) and dangling referents (fragments or pronouns whose subject isn't visible in-sentence) — the two drafting habits owners most reliably flag as "still feels AI".

...
- **Rulebook and casebook are different documents with different readers.** The rules files (base voice, variants) are model-facing: each class stated to stand on its own, with at most one generic illustration and a provenance tag. The ledger is the casebook: maintainer-facing, holding the verbatim instances, reviewer quotes and artifact history — and it stays OUT of the drafting/review load path (the router points sessions at it only for maintenance). Dense case history in working context turns rule-application into document recognition ("this is the piece the owner reviewed"), which biases review toward the remembered list, reads as memorisation to the owner, and makes fair testing of the package impossible. Recognition of previously reviewed material is never announced and never substitutes for a fresh rules pass.
